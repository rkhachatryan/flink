...
class IncrementalSnapshotTrackerImpl implements IncrementalSnapshotTracker {
    ...
    public IncrementalSnapshotTrackerImpl(
            Collection<KeyedStateHandle> stateHandles, StateHandleHelper stateHandleHelper) {
        this.confirmed = new IncrementalSnapshot(emptyMap(), stateHandleHelper.init(stateHandles));
        this.stateHandleHelper = stateHandleHelper;
    }

    @Override
    public IncrementalSnapshot getCurrentBase() {
        return confirmed;
    }
    ...
    public void track(
        ...
        checkState(!unconfirmedSnapshots.containsKey(checkpointId));
        unconfirmedSnapshots.put(
                checkpointId,
                new UnconfirmedSnapshot(
                        new IncrementalSnapshot(
                                snapshot.getAllVersions(),
                                stateHandleHelper.rebuildWithPlaceHolders(
                                        snapshot.getStateSnapshot())),
                        confirmCallbacks));
    }

    @Override
    public void confirmVersion(long checkpointId) {
        ...
        unconfirmedSnapshot.confirm();
        confirmed = unconfirmedSnapshot.snapshot;
