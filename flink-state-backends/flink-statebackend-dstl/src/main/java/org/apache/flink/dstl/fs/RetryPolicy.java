...
/**
 * Retry policy to use by DSTL. Aimed to curb tail latencies.
 */
@Internal
public interface RetryPolicy {
	...
	/**
	 * @return delay in millis before the next attempt. Negative means no retry, zero means no delay.
	 */
	long retryAfter(int failedAttempt, Exception exception);

	int maxAttempts();

	RetryPolicy NONE = new RetryPolicy() {
		@Override
	...
	class FixedRetryPolicy implements RetryPolicy {
		...
		public String toString() {
			return "timeout=" + timeout + ", maxAttempts=" + maxAttempts + ", delay=" + delayAfterFailure;
		}
	}
}
