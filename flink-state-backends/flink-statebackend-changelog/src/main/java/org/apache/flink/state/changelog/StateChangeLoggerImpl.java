...
import static org.apache.flink.state.changelog.StateChangeLoggerImpl.StateChangeOperation.METADATA;
...
@NotThreadSafe
class StateChangeLoggerImpl<Key, State, Ns> implements StateChangeLogger<State, Ns> {
    ...
    private final RegisteredStateMetaInfoBase metaInfo;
    private boolean metaDataWritten = false;

    StateChangeLoggerImpl(
    ...
    public void stateUpdated(State newState, Ns ns) throws IOException {
        if (newState == null) {
            ...
            log(SET, out -> valueSerializer.serialize(newState, out), ns);
        }
    }

    @Override
    public void stateAdded(State addedState, Ns ns) throws IOException {
        log(ADD, out -> valueSerializer.serialize(addedState, out), ns);
    }
    ...
    enum StateChangeOperation {
        ...
        static Map<Byte, StateChangeOperation> byCodes =
        ...
        public static StateChangeOperation byCode(byte opCode) {
            return Preconditions.checkNotNull(byCodes.get(opCode), Byte.toString(opCode));
        }
    }
}
