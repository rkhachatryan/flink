...
@Internal
public class OldContinuousFileReaderOperator<OUT> extends AbstractStreamOperator<OUT>
		implements OneInputStreamOperator<TimestampedFileInputSplit, OUT>, OutputTypeConfigurable<OUT> {

	private static final long serialVersionUID = 1L;

	private static final Logger LOG = LoggerFactory.getLogger(OldContinuousFileReaderOperator.class);
	...
	public OldContinuousFileReaderOperator(FileInputFormat<OUT> format) {
				...
				while (this.isRunning) {
					...
					synchronized (checkpointLock) {
						...
						if (this.format instanceof CheckpointableInputFormat && currentSplit.getSplitState() != null) {
							...
							((CheckpointableInputFormat<TimestampedFileInputSplit, Serializable>) this.format).reopen(currentSplit, currentSplit.getSplitState());
						} else {
