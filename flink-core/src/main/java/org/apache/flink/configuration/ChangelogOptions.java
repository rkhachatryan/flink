...
public class ChangelogOptions {
    ...
    public static final ConfigOption<Duration> PERIODIC_MATERIALIZATION_INTERVAL =
            ConfigOptions.key("state.backend.periodic-materialize.interval")
                    ...
                    .withDescription(
                            "Defines the interval in milliseconds to perform periodic materialization for state backend.");
    ...
    public static final ConfigOption<Boolean> PERIODIC_MATERIALIZATION_ENABLED =
            ConfigOptions.key("state.backend.periodic-materialize.enabled")
                    ...
                    .defaultValue(false)
                    .withDescription(
                            "Whether to perform periodic materialization for state backend.");

    @Documentation.Section(Documentation.Sections.STATE_BACKEND_CHANGELOG)
    public static final ConfigOption<Integer> MATERIALIZATION_MAX_ALLOWED_FAILURES =
            ConfigOptions.key("state.backend.periodic-materialize.enabled")
                    .intType()
                    .defaultValue(3)
                    .withDescription("Max allowed number of consecutive materialization failures.");
