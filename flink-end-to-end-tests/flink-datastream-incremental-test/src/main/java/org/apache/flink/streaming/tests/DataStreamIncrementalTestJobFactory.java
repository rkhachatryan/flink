 ...
 * </ul>
 */
public class DataStreamIncrementalTestJobFactory {
    ...
    private static void setupStateBackend(
        ...
        StateBackend backend = getBackend(env, pt);

//        if (pt.getConfiguration().get(CheckpointingOptions.ENABLE_STATE_CHANGE_LOG)) {
//            FsStateChangelogWriterFactory factory = new FsStateChangelogWriterFactory();
//            factory.configure(pt.getConfiguration());
//            backend = new ChangelogStateBackend(backend, factory);
//        }
//
        env.setStateBackend(backend);
    }

    private static StateBackend getBackend(StreamExecutionEnvironment env, ParameterTool pt) throws IOException {
        ...
        if ("file".equalsIgnoreCase(stateBackend)) {
            ...
            return new FsStateBackend(URI.create(checkpointDir), asyncCheckpoints);
            ...
            return new RocksDBStateBackend(checkpointDir, incrementalCheckpoints);
		} else {
