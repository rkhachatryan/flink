...
import org.apache.flink.runtime.state.heap.inc.IncrementalHeapSnapshotStrategy.StateTableVersions;
...
class IncrementalHeapSnapshotResultCallable<K> extends HeapSnapshotResultCallable<K> {
	...
	protected SnapshotResult<KeyedStateHandle> callInternal() throws Exception {
		...
		onSuccess.accept(new PreviousSnapshot(result, collectVersions()));
	...
	protected void writeSnapshot(int keyGroupId, StateSnapshot.StateKeyGroupWriter writer, DataOutputViewStreamWrapper dov, StateUID stateUID) throws IOException {
		if (writer instanceof IncrementalCopyOnWriteStateTableSnapshot) {
			Preconditions.checkState(previousMaybe.isPresent());
			//noinspection rawtypes
			((IncrementalCopyOnWriteStateTableSnapshot) writer).writeStateDiffInKeyGroup(
				dov,
				keyGroupId,
	...
	private StateTableVersions collectVersions() {
		...
		for (Map.Entry<StateUID, StateSnapshot> e : cowStateStableSnapshots.entrySet()) {
			if (e.getValue() instanceof IncrementalCopyOnWriteStateTableSnapshot) {
				IncrementalCopyOnWriteStateTableSnapshot<?, ?, ?> s = (IncrementalCopyOnWriteStateTableSnapshot<?, ?, ?>) e.getValue();
				...
				stateVersions.put(e.getKey(), s.collectVersions()); // todo: consider removing
			}
		}
