...
/** TBFD: javadoc. */
public interface StateJournalFactory<S, D extends StateDiff<S>, J extends StateJournal<S, D>> {

	J createJournal(S state, boolean wasCleared, boolean isStateExposed);

	static <S> StateJournalFactory<S, StateDiff<S>, StateJournal<S, StateDiff<S>>> noOp() {
		return (state, wasCleared, isStateExposed) -> new StateJournal<S, StateDiff<S>>() {
			@Override
