...
class IncrementalHeapSnapshotStrategy<K> extends HeapSnapshotStrategy<K> {
	...
	protected StateSnapshot getStateSnapshot(StateSnapshotRestore state, long checkpointId, CheckpointOptions checkpointOptions) {
		...
		boolean canSnapshotIncrementally = numIncremental < maxIncremental && !checkpointOptions.getCheckpointType().isSavepoint() && checkpointId == lastConfirmedCheckpoint + 1 && state instanceof CopyOnWriteStateTable;
		...
		return canSnapshotIncrementally ?
			// todo: remove cast
			((CopyOnWriteStateTable) state).incrementalStateSnapshot() :
	...
	static class StateMapVersions {
		private final Map<Integer, Integer> kgMapVersion;

		StateMapVersions(Map<Integer, Integer> kgMapVersion) {
