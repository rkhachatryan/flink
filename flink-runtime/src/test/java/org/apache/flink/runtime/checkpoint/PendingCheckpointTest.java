...
public class PendingCheckpointTest {
	...
	public void testCanBeSubsumed() throws Exception {
		...
		try {
			pending.abort(CheckpointFailureReason.CHECKPOINT_SUBSUMED, () -> {}, Executors.directExecutor());
	...
	public void testCompletionFuture() throws Exception {
		...
		pending.abort(CheckpointFailureReason.CHECKPOINT_DECLINED, () -> {}, Executors.directExecutor());
	...
	public void testAbortDiscardsState() throws Exception {
		...
		pending.abort(CheckpointFailureReason.CHECKPOINT_DECLINED, () -> {}, executor);
		...
		pending.abort(CheckpointFailureReason.CHECKPOINT_EXPIRED, () -> {}, executor);
		// execute asynchronous discard operation
		executor.runQueuedCommands();
		verify(state, times(1)).discardState();
		...
		pending.abort(CheckpointFailureReason.CHECKPOINT_SUBSUMED, () -> {}, executor);
		// execute asynchronous discard operation
		executor.runQueuedCommands();
		verify(state, times(1)).discardState();
	}
	...
	public void testPendingCheckpointStatsCallbacks() throws Exception {
...

		{
			...
			pending.abort(CheckpointFailureReason.CHECKPOINT_EXPIRED, () -> {}, Executors.directExecutor());
			verify(callback, times(1)).reportFailedCheckpoint(anyLong(), any(Exception.class));
		}
	}
	...
	public void testSetCanceller() throws Exception {
		...
		aborted.abort(CheckpointFailureReason.CHECKPOINT_DECLINED, () -> {}, Executors.directExecutor());
	...
	public void testDisposeDisposesCoordinatorStates() throws Exception {
		...
		checkpoint.abort(CheckpointFailureReason.CHECKPOINT_EXPIRED, () -> {}, Executors.directExecutor());
