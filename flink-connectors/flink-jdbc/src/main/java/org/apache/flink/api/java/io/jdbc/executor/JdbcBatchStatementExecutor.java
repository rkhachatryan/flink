...
public interface JdbcBatchStatementExecutor<T> {
	...
	static JdbcBatchStatementExecutor<Row> simpleRow(String sql, int[] fieldTypes) {
		return simple(sql, ParameterSetter.forRow(fieldTypes));
	}

	static <T> JdbcBatchStatementExecutor<T> simple(String sql, ParameterSetter<T> paramSetter) {
