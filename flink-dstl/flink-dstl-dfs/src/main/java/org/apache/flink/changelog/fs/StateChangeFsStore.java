...
class StateChangeFsStore implements StateChangeStore {
    ...
    private final int bufferSize = 1024 * 1024 * 10; // todo configure externally

    public StateChangeFsStore(Path basePath, FileSystem fileSystem, boolean compression) {
    ...
    @Override
    public void save(Collection<StateChangeSetUpload> changeSets) throws IOException {
    ...
    private void upload(
        ...
        if (offsets.isEmpty()) {
            ...
            // todo: sync copmletion of all updates in a single file?
            offsets.forEach(
                    (upload, offset) ->
                            upload.complete(
                                    new StoreResult(
