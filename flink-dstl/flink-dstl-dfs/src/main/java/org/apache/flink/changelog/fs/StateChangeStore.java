...
interface StateChangeStore extends AutoCloseable {
    ...
    void save(Collection<StateChangeSetUpload> changeSets) throws IOException;

    static StateChangeStore fromConfig(ReadableConfig config) throws IOException {
        StateChangeFsStore store =
                createSimpleFsStore(
                        new Path(config.get(BASE_PATH)), config.get(COMPRESSION_ENABLED));
    ...
    static StateChangeFsStore createSimpleFsStore(Path basePath, boolean compression)
            throws IOException {
        return new StateChangeFsStore(basePath, basePath.getFileSystem(), compression);
    }
