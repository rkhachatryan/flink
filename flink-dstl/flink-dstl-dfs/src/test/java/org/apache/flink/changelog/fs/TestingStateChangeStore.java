...
class TestingStateChangeStore implements StateChangeStore {
    ...
    @Override
    public void save(Collection<StateChangeSetUpload> changeSets) {
    ...
    public void completeAndReset() {
        for (StateChangeSetUpload upload : saved) {
            upload.complete(new StoreResult(new ByteStreamStateHandle("", new byte[1]), 0L, SequenceNumber.FIRST, 100L));
        }
        reset();
    }
}
