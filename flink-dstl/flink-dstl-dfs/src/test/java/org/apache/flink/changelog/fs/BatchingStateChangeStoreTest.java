...
public class BatchingStateChangeStoreTest {
    ...
    public void testErrorHandling() throws Exception {
        ...
        try (BatchingStateChangeStore store =
            ...
            try {
                ...
                changes.forEach(c -> assertTrue(c.getCurrentUpload().getStoreResultFuture().isCompletedExceptionally()));
            }
        }
    }
    ...
    private final void assertSaved(
        ...
        assertEquals(
                Arrays.stream(expected).flatMap(Collection::stream).map(StateChangeSet::getCurrentUpload).collect(toList()),
